 ...
 * I modified a bit the code to be able to deal with the user's answer after a
...
class CommandParser {
  ...
  private Scanner questionParser;

  CommandParser(VideoPlayer videoPlayer) {
    this.videoPlayer = videoPlayer;
    questionParser = new Scanner(System.in);
  }
    ...
    switch (command.get(0).toUpperCase()) {
      ...
      case "SEARCH_VIDEOS":
        try {
          ...
          String answer = questionParser.nextLine();
          if(this.videoPlayer.isQuestionAsked()) this.videoPlayer.searchVideosQuestion(answer);
        } catch (ArrayIndexOutOfBoundsException e) {
